...
const totalNumbersElement = document.querySelector(".total-progress-numbers");
...
function createEventCard(event) {
    ...
    const roles_min = roles.reduce((total, role) => total + role.min_count, 0);
    ...
    <div class="progress-container">
        <div class="progress-bar">
            <div class="progress-fill" style="width: ${roles_min > 0 ? (roles_now / roles_min) * 100 : 0}%;"></div>
        </div>
    </div>
    <p class="progress-numbers">(${roles_now}/${roles_min})</p>
    <button class="event-details-button" data-event-id="${event._id}">View Details</button>
`;
...
eventContainer.addEventListener("click", (event) => {
    if (event.target.classList.contains("show-details-button")) {
        ...
        fetch(`http://127.0.0.1/api/events/${eventId}`)
...
function updateEventData() {
    ...
    fetch("http://127.0.0.1/api/events/")
        .then(response => response.json())
        .then(data => {
...
function updateTotalRoles(eventData) {
    ...
    eventData.forEach(event => {
        totalRolesMin += event.roles.reduce((total, role) => total + role.min_count, 0);
        totalRolesNow += event.roles.reduce((total, role) => total + role.count, 0);
    });

    totalNumbersElement.textContent = `(${totalRolesNow}/${totalRolesMin})`;
    
    const totalPercentage = totalRolesMin > 0 ? (totalRolesNow / totalRolesMin) * 100 : 0;
